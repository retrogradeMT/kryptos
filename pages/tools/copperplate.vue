...
// Support an arbitrary rotation angle in degrees.  The base rotation is
// determined by the orientation (normal/tree/roots), then this custom
// angle is added on top.  Positive values rotate clockwise, negative
// values rotate counter‑clockwise.
const customAngle = ref(0)

// Compute the net rotation angle for display.  We apply only the
// user‑selected custom angle here; the fixed TREE/ROOTS rotations are
// handled by rotating the underlying data via orientedGrid.  This
// prevents double rotations and keeps the right side at the top for
// the TREE orientation.
const totalAngle = computed(() => {
  return customAngle.value
})

// Use the global state so we can push selected lines back into the
// current text.  We update currentText.value directly.
const { currentText } = useGlobalState()

// Send the rows between the first and last selected cell to the global
// state.  This mirrors copyRowsBetween() but instead of copying to
// clipboard or opening a new window we assign the value to
// currentText.value.  If the selection is invalid we show an alert.
function sendRowsBetweenToGlobal() {
  const text = getRowsBetweenText()
  if (!text) {
    alert('Pick any two cells to define start/end rows')
    return
  }
  currentText.value = text
}

// Send the current selection (path, row span or column span) into the
// global state.  The mode argument matches sendSelectionToDMS() above.
function sendSelectionToGlobal(mode = 'path') {
  let text = ''
  if (mode === 'row') {
    if (selectedCells.value.length < 2) { alert('Pick two cells in same row'); return }
    const [a, b] = selPath.value
...
// Styles applied to the grid container and each cell to perform the
// rotation.  The container is rotated by totalAngle degrees, and each
// character is counter‑rotated to remain upright.  We define these
// computed properties here so they update reactively when the angle
// changes.
const gridContainerStyle = computed(() => ({
  transform: `rotate(${totalAngle.value}deg)`,
  transformOrigin: 'center',
  overflow: 'visible',
}))

const cellRotationStyle = computed(() => ({
  // Counter‑rotate each character so it remains upright.  We deliberately
  // leave display and dimensions untouched so that the original flex
  // layout can control cell widths and heights, preserving alignment on
  // ragged rows.
  transform: `rotate(${-totalAngle.value}deg)`,
  transformOrigin: 'center',
}))
      ...
      <div class="xl:pl-96">
        <div class="px-4 py-10 sm:px-6 lg:px-8 lg:py-6">
          ...
          <div class="mb-6 min-w-2/5">
            ...
            >
              <div>
                <div v-for="(row, rIdx) in paddedGrid" :key="`row-${rIdx}`" class="flex">
                  <!-- left-edge bark marker gutter -->
                  <span
                    class="w-3 mr-1 text-right select-none" :style="{
                  ...
                  <span
                    v-for="(char, cIdx) in row" :key="`cell-${rIdx}-${cIdx}`"
                    class="px-2 py-1 leading-none text-kryptos"
                    :class="{ 'opacity-30 select-none pointer-events-none': char === '#' }" :style="[
                      cellRotationStyle,
                      char === '#' ? {} : highlightStyleFor(rIdx, cIdx, char),
                    ]"
                  >
                    {{ char }}
                  </span>
...
VTTMZFPKWGDKZXTJCDIGKUHUAUEKCAR
              ...
              <div class="flex flex-wrap items-center gap-4 mt-2 text-sm text-gray-500">
                ...
                <!-- Custom rotation angle control -->
                <label class="flex items-center gap-2 ml-4">
                  Rotate:
                  <input v-model.number="customAngle" type="range" min="-180" max="180" step="1" class="w-40" />
                  <span class="whitespace-nowrap">{{ customAngle }}°</span>
                </label>
              </div>
            </div>
            ...
            <div class="p-1 mt-2 border border-primary">
              ...
              <div class="flex flex-wrap gap-2 mb-2">
                ...
                <!-- Buttons to push selected text into the global state -->
                <button class="px-2 py-1 text-sm" @click="sendSelectionToGlobal('path')">
                  Send Path → Global
                </button>
                <button class="px-2 py-1 text-sm" @click="sendSelectionToGlobal('row')">
                  Send Row Span → Global
                </button>
                <button class="px-2 py-1 text-sm" @click="sendSelectionToGlobal('col')">
                  Send Col Span → Global
                </button>
                <button class="px-2 py-1 text-sm" @click="sendRowsBetweenToGlobal">
                  Send Rows Between → Global
                </button>
              </div>
              <textarea
