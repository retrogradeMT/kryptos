  ...
  <div :class="printMode ? 'bg-white text-black' : ''" class="min-h-full">
    <div class="px-4 py-6">
      <PageTitle class="uppercase" title="Kryptos Copperplate" size="lg" :display="true">
        <template #subtitle>Static grid explorer • selectable rows/cols • TREE/ROOTS orientation</template>
      </PageTitle>

      <!-- Hidden seed (fallback) -->
      <textarea ref="seedTA" aria-hidden="true" tabindex="-1"
        style="position:absolute;left:-10000px;top:-10000px;width:1px;height:1px;opacity:0;"></textarea>

      <!-- Grid -->
      <section class="mt-4">
        <h3 class="mb-1 font-semibold text-green-400">Static Grid</h3>
        <div
          ...
          <div>
            <div v-for="(row, rIdx) in orientedGrid" :key="`row-${rIdx}`" class="flex">
              ...
              <span class="w-3 mr-1 text-right select-none"
                :style="{ color: barkMarkerRows.has(rIdx) ? (printMode ? '#000' : '#f59e0b') : 'transparent' }"
                title="Bark mark">●</span>

              <span v-for="(ch, cIdx) in row" :key="`c-${cIdx}`" class="px-1 cursor-pointer select-none" :style="{
                ...
                }" @click="handleCellClick({ char: ch, row: rIdx, col: cIdx })">{{ ch }}</span>
            </div>
          </div>
        </div>
      </section>
      ...
      <section class="mt-3">
        <div class="flex flex-wrap gap-2 text-sm">
          <button class="px-2 py-1" @click="clearSelection">Clear</button>
          <button class="px-2 py-1" @click="removeLast" :disabled="!selectedCells.length">Remove Last</button>
          <button class="px-2 py-1" @click="copySelectedPath" :disabled="!selectedCells.length">Copy Path</button>
          <button class="px-2 py-1" @click="copyRowSpan" :disabled="selectedCells.length < 2">Copy Row Span</button>
          <button class="px-2 py-1" @click="copyColSpan" :disabled="selectedCells.length < 2">Copy Col Span</button>
          <button class="px-2 py-1" @click="copyRowsBetween" :disabled="selectedCells.length < 2">Copy Rows
            Between</button>
          <button class="px-2 py-1" @click="sendRowsBetweenToTrans" :disabled="selectedCells.length < 2">Send Rows
            Between
            → Transposition</button>
        </div>
        <textarea :value="selectedCells.map(c => `${c.char} (r${c.row},c${c.col}) #${c.order}`).join('\n')" rows="5"
...
watch(() => route.query.text, (v) => {
  if (typeof v === 'string') STATIC_GRID_TEXT.value = decodeURIComponent(v)
})
...
function padToRect(grid: string[][], padChar = ' ') {
  ...
  return grid.map(row => (row.length < Cmax ? row.concat(Array(Cmax - row.length).fill(padChar)) : row))
}
...
const orientedGrid = computed(() => {
  ...
  if (!g.length) return [] as string[][]
  if (gridOrientation.value === 'tree') return rotateCCW(g)
  if (gridOrientation.value === 'roots') return rotateCW(g)
  return g
})
...
const barkMarkerRows = computed(() => {
  const R = orientedGrid.value.length; if (!R) return new Set<number>()
...
function scanRows(grid:string[][], q:string, includeWrap=true, backwards=false){
  const res: number[][][] = []; if (!grid.length || !q) return res; const L = q.length
  for(let r=0;r<grid.length;r++){
    const C = grid[r].length
    for(let c0=0;c0<C;c0++){
      if(!includeWrap){ if(!backwards && c0+L>C) continue; if(backwards && c0-(L-1)<0) continue }
...
function scanCols(grid:string[][], q:string, includeWrap=true, backwards=false){
  const res: number[][][] = []; if (!grid.length || !q) return res; const L=q.length; const R=grid.length; const Cmax=Math.max(...grid.map(r=>r.length))
  for(let c=0;c<Cmax;c++){
    for(let r0=0;r0<R;r0++){
      if(!includeWrap){ if(!backwards && r0+L>R) continue; if(backwards && r0-(L-1)<0) continue }
      let ok=true; const cells: number[][]=[]
      for(let k=0;k<L;k++){
  ...
  for(let r0=0;r0<R;r0++){
    for(let c0=0;c0<Cmax;c0++){
      ...
      for(let k=0;k<L;k++){
        ...
        if(!includeWrap && (r<0||r>=R||c<0||c>=Cmax)) break
      }
      if(ok && cells.length===L) res.push(cells)
    }
  }
  return res
}
...
const findMatches = computed(() => {
  ...
  if(!g.length||!q) return [] as number[][][]
  const wrap = findWrap.value; const back = findBack.value; const mode = findMode.value
  const out: number[][][] = []
  if(mode==='rows'||mode==='all'){ out.push(...scanRows(g,q,wrap,false)); if(back) out.push(...scanRows(g,q,wrap,true)) }
  if(mode==='cols'||mode==='all'){ out.push(...scanCols(g,q,wrap,false)); if(back) out.push(...scanCols(g,q,wrap,true)) }
  if(mode==='diagDown'||mode==='all'){ out.push(...scanDiag(g,q,+1,+1,wrap)); if(back) out.push(...scanDiag(g,q,-1,-1,wrap)) }
  if(mode==='diagUp'||mode==='all'){ out.push(...scanDiag(g,q,-1,+1,wrap)); if(back) out.push(...scanDiag(g,q,+1,-1,wrap)) }
  return out
})
...
function getCharAt(r:number,c:number){ const g=orientedGrid.value; if(r<0||r>=g.length) return ''; const row=g[r]; if(!row||c<0||c>=row.length) return ''; return row[c] }
function getRow(r:number){ const g=orientedGrid.value; return (r>=0&&r<g.length) ? g[r].join('') : '' }

function copy(text:string){ try{ navigator.clipboard.writeText(text) }catch(e){ console.warn('Clipboard failed', e) } }

function clearSelection(){ selectedCells.value = [] }
function removeLast(){ if(selectedCells.value.length) selectedCells.value.pop() }
function copySelectedPath(){ const s = selectedCells.value.map(c=>c.char||'').join(''); if(s) copy(s) }
function copyRowSpan(){ if(selectedCells.value.length<2) return; const [a,b] = selectedCells.value.slice().sort((x,y)=>x.order-y.order); if(a.row!==b.row) return; const r=a.row; const c0=Math.min(a.col,b.col); const c1=Math.max(a.col,b.col); let out=''; for(let c=c0;c<=c1;c++) out+=getCharAt(r,c); copy(out) }
function copyColSpan(){ if(selectedCells.value.length<2) return; const [a,b] = selectedCells.value.slice().sort((x,y)=>x.order-y.order); if(a.col!==b.col) return; const c=a.col; const r0=Math.min(a.row,b.row); const r1=Math.max(a.row,b.row); let out=''; for(let r=r0;r<=r1;r++) out+=getCharAt(r,c); copy(out) }
function rowsBetween(){ if(selectedCells.value.length<2) return ''; const [a,b]=selectedCells.value.slice().sort((x,y)=>x.order-y.order); const r0=Math.min(a.row,b.row); const r1=Math.max(a.row,b.row); const lines:string[]=[]; for(let r=r0;r<=r1;r++) lines.push(getRow(r)); return lines.join('\n') }
function copyRowsBetween(){ const t = rowsBetween(); if(t) copy(t) }
// Replace sendRowsBetweenToDMS with sendRowsBetweenToTrans
function sendRowsBetweenToTrans(){
  const t = rowsBetween(); if(!t) return;
